		...
		for (UStaticMeshComponent* SMC : SMCs)
		{
			...
			for (int32 SectionIdx = 0; SectionIdx < LOD0.Sections.Num(); ++SectionIdx)
			{
				...
				if (MatInterface)
				{
					...
					MatInterface->GetUsedTextures(UsedTextures, EMaterialQualityLevel::High, false, ERHIFeatureLevel::SM5, true);
		...
		for (UPrimitiveComponent* PrimComp : PrimComps)
		{
			...
			for (int32 MatIdx = 0; MatIdx < NumMaterials; ++MatIdx)
			{
				...
				if (Entry.MaterialName.IsEmpty())
				{
					...
					MatInterface->GetUsedTextures(Textures, EMaterialQualityLevel::High, false, ERHIFeatureLevel::SM5, true);
